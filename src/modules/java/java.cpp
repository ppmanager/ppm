...
bool java(int argc, char* argv[])
{
	...
	if(helpLib)
	{
		char path[strlen(appRoot) + strlen("_ppm\\java_libs\\*") + 2];
		sprintf(path, "%s\\_ppm\\java_libs\\*", appRoot);
	...
	char libPath[100] = "_ppm\\java_libs\\";
